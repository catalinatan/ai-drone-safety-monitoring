...
def compute_seg_aggregate(
    seg_scores: dict[str, dict[str, float]], seg_keys: list[str], weights: list[float]
) -> dict[str, float]:
...
def plot_grouped_bars(
    ...
    for i, task in enumerate(seg_keys):
        ...
        bars = ax1.bar(x + offset, vals, width, label=f"{task.capitalize()} IoU", color=colours[i])
    ...
    bars = ax1.bar(
        x + offset, agg_vals, width, label="Seg Aggregate (WHM)", color="black", alpha=0.7
    )
...
def plot_radar(
    ...
    for vi, variant in enumerate(variants):
        ...
        vals += vals[:1]
        ax.plot(
            angles, vals, "o-", linewidth=1.5, label=labels[vi], color=colours[vi], markersize=5
        )
        ax.fill(angles, vals, alpha=0.08, color=colours[vi])

    ax.set_thetagrids(
        np.degrees(angles[:-1]), [t.capitalize() + " IoU" for t in seg_keys] + ["Human F1"]
    )
    ax.set_ylim(0, 0.8)
...
def plot_dual_pareto(
    ...
    variants = [v for v in VARIANT_ORDER if v in latency and (v in seg_agg or v in human_scores)]
    labels = [VARIANT_LABELS[VARIANT_ORDER.index(v)] for v in variants]
    ...
    for ax, score_dict, ylabel, title in [
        ...
        for i in frontier:
            ax.scatter(
                [xs[i]], [ys[i]], s=180, facecolors="none", edgecolors="red", linewidths=2, zorder=6
            )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Cross-task model selection and visualisation")
    parser.add_argument(
        "--seg-weights",
        type=float,
        nargs="+",
    ...
    seg_keys = [t for t, *_ in SEG_TASK_CONFIGS if any(t in s for s in seg_scores.values())]
    ...
    if len(seg_weights) != len(seg_keys):
        print(
            f"ERROR: --seg-weights expects {len(seg_keys)} values "
